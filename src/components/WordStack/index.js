...
const WordStack = ({input,setInput,setScore,setLevel,setMultiplier,multiplier,setShowGameOver,wordStack,setWordStack,speed,setSpeed,setWPM}) => {
    ...
    },[input,multiplier,setInput,setMultiplier,setScore,setWPM,setWordStack,wordStack])

    return (
        <div className="word-stack">
            <TransitionGroup component={null}>
                {wordStack.length!==0 && wordStack.map((word,index) =>{
                    return (
                        <CSSTransition key={word} timeout={500} classNames="word">
                        <div className="word">{word?.split("").map((letter,index) =>{
                            let color;
                            if(input.split("")[index] === letter && word.startsWith(input)){
                                if(input === word[0]){
                                     t1.current = Date.now()
                                }
